...
export async function POST(request: NextRequest, context: any) {
  try {
    ...
    if (!files || !files.length) return badRequest('No files provided')

    const { mkdir, writeFile } = await import('fs/promises')
    const { join } = await import('path')
    const dir = join(process.cwd(), 'storage', 'tickets', id)
    ...
    for (const f of files) {
      ...
      const att = await (prisma as any).ticketAttachment.create({
        data: {
          ticketId: id,
          userId,
          fileName,
