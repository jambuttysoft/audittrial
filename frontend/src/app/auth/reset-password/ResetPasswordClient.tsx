...
export default function ResetPasswordClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) { setSuccess(true); setTimeout(() => { router.push('/') }, 3000) } else { setError(data.error || 'Password reset failed') }
        ...
        <div className="auth-container">
          <div className="auth-wrapper" style={{ justifyContent: 'center', alignItems: 'center' }}>
            <div style={{ maxWidth: '400px', width: '100%', padding: '40px' }}>
              ...
              <div className="auth-switch">Remember your password? <a onClick={() => router.push('/')}>Sign in</a></div>
            </div>
          </div>
        </div>
      )}
